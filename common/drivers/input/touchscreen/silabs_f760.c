...
static int silabs_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
    ...
    { 
        ...
        printk("[TSP] lyon.cho taeyang 0x08 update : %d\n", __LINE__ );
	    local_irq_enable();
    ...
    else if (( buf_firmware[2] == YTE_MODULE_VER_NEW)&&(buf_firmware[0] < FW_VER_NEW))
    { 
         TSP_MODULE_ID =  buf_firmware[2];
         PHONE_VER = FW_VER_NEW;
	     local_irq_disable();
		 ret = Firmware_Download();	
         printk("[TSP] enable_irq : %d\n", __LINE__ );
	     local_irq_enable();
